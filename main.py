...
#     background_tasks: BackgroundTasks,
#     files: list[UploadFile] = File(...),
#     chat_id: str = None,  # 👈 YEH ADD KARO - optional parameter
#     user=Depends(get_current_user)
# ):

#     supabase = get_supabase_user_client(user["access_token"])
    
#     # 🔥 FIX: Agar chat_id diya hai toh use karo, nahi toh naya banao
#     if chat_id:
#         # Check karo chat exist karta hai aur user ka hai
...
#         else:
#             # Agar chat exist nahi karta toh naya banao
#             new_chat = create_chat(supabase, user["id"])
#             final_chat_id = new_chat["id"]
#             print(f"📤 Chat not found, created new: {final_chat_id}")
#     else:
#         # Agar chat_id nahi diya toh naya banao
#         new_chat = create_chat(supabase, user["id"])
#         final_chat_id = new_chat["id"]
...
#         "message": "Uploading started",
#         "uploaded_files": uploaded_count,
#         "chat_id": final_chat_id,  # 👈 YEH RETURN KARO
#         "status": "processing"
#     }
...
#     # 🔥 CRITICAL FIX: Add logging to see who's calling this
#     import traceback
#     print("="*50)
